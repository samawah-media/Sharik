...
| `client_viewer` | متابع للقراءة فقط | مشاهدة وتنزيل الملفات والحالة المصرح بها | اعتماد، طلب تعديل، تعليق، أو رفع ملف | Confirmed |
...
| لا تدخل النسخة انتظار العميل إذا كان النص مجرد placeholder؛ يلزم نص حقيقي أو ملف مرئي غير فارغ | Confirmed in Spec 015 |
| الملفات الداخلية لا تظهر للعميل | Confirmed |
| الملفات النهائية تظهر بعد التسليم أو حسب رؤية الملف | Confirmed |
| ملف يرفعه `client_admin` أو `client_approver` يصنف كملف عميل ويظهر للفريق المصرح؛ `client_viewer` لا يرفع | Confirmed in Spec 015 |
...
| هل يستطيع `client_viewer` كتابة تعليق غير رسمي؟ | حُسم في Spec 015: لا؛ Viewer للقراءة فقط. |
...
| هل يستطيع client_viewer كتابة تعليق غير رسمي؟ | حُسم في Spec 015: لا؛ أي Reviewer مستقبلي يحتاج دورا وصلاحية مستقلين. |
