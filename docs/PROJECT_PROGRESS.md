...
## Spec 015 X009-A local ready for Codex review — 2026-07-21

Status: `X009_A_LOCAL_READY_FOR_REVIEW_HOSTED_PREVIEW_VERIFY_PENDING_BYPASS`.
...
Defect S015-P1-088 is registered and locally fixed. The isolated Preview `https://shrik-ou7tduzns-samawahs-projects.vercel.app` for commit `2373700` is Ready and non-Production (target=preview) and inherits the UAT Supabase env binding, but Vercel Deployment Protection currently redirects both the deployment URL and its git-tracking alias to `https://vercel.com/login`. X009-A-5 (browser reproduction through real Auth/UI) remains pending a Vercel protection bypass from the owner or a temporary authorized alias reassignment; the stable UAT alias was not modified per the prompt boundary. The owner-approved Spec 015 clarifications remain preserved unstaged. No Production deployment, stable UAT alias promotion, merge, external invitation, public signup, real customer data, or workbook commit occurred.
