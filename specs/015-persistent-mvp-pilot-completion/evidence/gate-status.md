...
| Migration gate | pending re-verification | The new migrations have not been applied to UAT in this continuation. No hosted mutation is authorized until local Docker-backed reset/RLS evidence is available and the reviewed migration inventory is rechecked. |
...
| Local UX rescue verification | partial | Typecheck, unit, integration, component, simulator RLS, build, targeted lint, diff check, and targeted fixture E2E pass; full lint/secret scan and all-project E2E are not claimed in this continuation. |
