...
| Hosted target verification | partial / blocked | Supabase linked target is `sharik-uat` and ACTIVE_HEALTHY. Vercel Preview deployment is Ready with target `preview`, not production. Browser access to the Preview redirects to Vercel login because Deployment Protection is enabled, and share/bypass generation failed from the available connector/CLI permissions. |
