...
| Hosted target verification | partial / blocked | Supabase linked target is `sharik-uat` and ACTIVE_HEALTHY. Vercel Preview deployment is Ready with target `preview`, not production. `vercel curl` can generate/use an internal automation bypass and returned the Arabic sign-in shell, but normal browser access redirects to Vercel login because Deployment Protection is enabled. No safe shareable team URL or exposed bypass secret is available from current tooling. |
...
| Synthetic Hadna seed | green | Idempotent S015 hosted seed created one synthetic contract, package, package line, deliverable, version, file metadata row, client-visible comment, and SLA segment. Evidence is count/category-only. |
| Team access | blocked | UAT role counts show tenant admin, account manager, client viewer, and client approver categories only. No content_writer/designer category exists for the full H008 matrix, and browser sign-in is blocked by Vercel Deployment Protection. Stop condition: `TEAM_ACCESS_INPUT_REQUIRED` plus Vercel protection bypass/disablement required. |
| Preview deployment | partial / blocked | Preview deployment built successfully and is Ready. Preview env contains `APP_ENV`, `NEXT_PUBLIC_SUPABASE_URL`, and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`; protected `/sign-in` access through `vercel curl` returns the Arabic shell and no service-role marker. Normal browser/team access is still blocked by Vercel Deployment Protection. Production remains untouched. |
| Hosted workflow/UX UAT | blocked | Cannot run actual hosted browser journey until Preview access is opened via Vercel protection bypass/disablement and approved tester role mapping covers the required personas. |
| T032 hosted evidence | blocked | T032 remains open. Rollback boundary is known, but hosted Team UAT cannot pass while Preview browser access and full tester mapping are unavailable. |
