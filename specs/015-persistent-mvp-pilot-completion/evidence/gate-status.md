...
| Hosted UAT | X007 Checkpoint 4 blocked at hosted persona smoke | Previously accepted UAT migration/import evidence remains valid. Process-local guard metadata allowed the hosted runner to list all 27 checks, and the public shell/service-credential leak checks passed across desktop, mobile, and RTL. Persona smoke remains blocked: the configured UAT entry serves 404 for the required Spec 015 `/work` and `/client/pending` routes after sign-in, while direct latest Preview exploration timed out on persona sign-in. Exact reviewed Preview/UAT routing or owner-side alias/protection correction is required before H008. H008-H010 and T032 remain open. |
...
| Team access | seven approved categories configured / hosted route smoke blocked | The secure ignored persona store contains management, account manager, assigned writer, assigned designer, unassigned internal negative tester, client approver, and client viewer categories. Browser automation can reach the public UAT shell, but the configured UAT entry does not serve the required Spec 015 role routes after sign-in, so persona access is not reverified and H008 is not started. |
| Preview deployment | exact-HEAD build historically green / UAT routing blocked | Historical Preview evidence remains accepted. In this resume, process-local hostname/category guards allowed hosted listing and boundary execution; no alias, deployment, or Production setting was changed. The configured UAT entry appears stale or mismatched because `/work` and `/client/pending` return 404 after sign-in. Direct latest Preview exploration remained Preview-only but timed out during persona smoke. |
| Hosted workflow/UX UAT | blocked before H008 | Hosted boundary checks passed 6/6 across desktop, mobile, and RTL for fixture-impersonation refusal and browser-side service-credential secrecy. The 21 persona route smoke checks failed on missing hosted role routes, and direct latest Preview exploration was stopped after repeated sign-in timeouts. This is environment/deployment routing evidence, not H008 PASS. |
