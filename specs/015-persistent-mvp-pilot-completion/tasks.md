...
- [x] X009-A Diagnose and permanently fix the online "تعذر حفظ المخرج بأمان" failure without starting the visual redesign, UAT cleanup, client onboarding wizard, or any data deletion.
  ...
  - [x] X009-A-2 Implement the smallest correct root-cause fix: identifier pre-validation on the action-layer owner/contributor inputs and an actionable Arabic `mapDeliverableWriteError` that surfaces 22P02 (and 23502/23503/23514) instead of the generic safe-save fallback, without weakening authorization, RLS, or the audited transactional RPC.
  - [x] X009-A-3 Add focused regression coverage for schema/form validation, error mapping, authorized management creation, unauthorized/cross-scope denial (covered by existing pgTAP), package capacity (covered by existing pgTAP), atomic rollback, idempotent replay, audit/ledger integrity, and persistent browser creation + reload.
