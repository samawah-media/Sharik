  ...
  return (
    <section className="grid gap-4" aria-labelledby="management-exceptions" dir="rtl">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
        {metrics.map(([label, count, tone]) => <article className="rounded-lg border border-border bg-surface p-3 shadow-xs" key={label}><Badge tone={tone}>{label}</Badge><p className="mt-2 text-xl font-semibold tabular-nums sm:text-2xl">{count}</p></article>)}
      </div>
      <div className="grid gap-3 lg:grid-cols-2">
        <section className="rounded-lg border border-border bg-surface p-3 shadow-xs" aria-labelledby="workload-heading"><h2 className="font-semibold" id="workload-heading">ضغط الفريق</h2>{workload.length ? <ul className="mt-2 grid gap-2">{workload.slice(0, 8).map(([name, count]) => <li className="flex min-h-11 items-center justify-between rounded-lg bg-background px-3" key={name}><span>{name}</span><Badge tone={count >= 5 ? "warning" : "muted"}>{count} نشط</Badge></li>)}</ul> : <p className="mt-2 text-sm text-muted">لا توجد أعمال نشطة.</p>}</section>
        <section className="rounded-lg border border-border bg-surface p-3 shadow-xs" aria-labelledby="client-progress-heading"><h2 className="font-semibold" id="client-progress-heading">تقدم العملاء والباقات</h2><ul className="mt-2 grid gap-2">{clients.map((client) => <li className="grid gap-2 rounded-lg bg-background p-3" key={client.clientId}><div className="flex items-center justify-between gap-2"><span className="font-semibold">{client.name}</span><span className="text-sm tabular-nums">{client.progress}%</span></div><div className="h-2 overflow-hidden rounded-full bg-border"><div className="h-full bg-accent" style={{ width: `${client.progress}%` }} /></div><div className="flex items-center justify-between text-xs text-muted"><span>{client.delivered} من {client.total} تم تسليمه</span><ButtonLink href={`/clients/${client.clientId}/commercial`} size="sm">ملف العميل</ButtonLink></div></li>)}</ul></section>
      </div>
      <section className="rounded-lg border border-border bg-surface p-3 shadow-xs" aria-labelledby="recent-decisions"><h2 className="font-semibold" id="recent-decisions">أحدث القرارات والتسليمات</h2>{recent.length ? <ol className="mt-2 grid gap-2">{recent.map((item) => <li className="min-h-11" key={item.id}><ButtonLink className="grid w-full gap-1 rounded-lg bg-background px-3 py-2 text-right transition-colors hover:bg-accent-soft focus-visible:outline-2 focus-visible:outline-offset-2 sm:grid-cols-[minmax(0,1fr)_auto]" href={`/clients/${item.clientId}/deliverables`} size="sm" variant="ghost"><span className="font-semibold">{item.name}</span><span className="text-xs text-muted">{clientNames[item.clientId]} · {deliverableStatusLabel(item.status)}</span></ButtonLink></li>)}</ol> : <p className="mt-2 text-sm text-muted">لا توجد قرارات حديثة.</p>}</section>
    </section>
  );
}
