...
test("client pending route is a real inbox and stays read-only for viewer", async ({
  page,
}) => {
  ...
  await expect(page.getByText("مخرج تجريبي آمن", { exact: true })).toHaveCount(
    1,
  );
  await expect(
...
test("client approver receives the decision controls for the current version", async ({
  page,
}) => {
